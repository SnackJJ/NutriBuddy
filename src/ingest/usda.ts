...
import type { CatalogSnapshot, FoodNutritionLookup } from "../lib/usda";
...
const CATEGORY_RULES: ReadonlyArray<{
  ...
  {
    category: "meat",
    pattern: /\b(chicken|beef|pork|turkey|steak|lamb|meat)\b/,
  },
  {
    category: "seafood",
    pattern: /\b(salmon|tuna|shrimp|cod|fish|seafood|prawn)\b/,
  },
  {
    category: "grain",
    pattern: /\b(rice|bread|pasta|oat|cereal|noodle|grain|flour)\b/,
  },
  {
    category: "vegetable",
    pattern:
      /\b(broccoli|spinach|carrot|tomato|potato|lettuce|onion|pepper|vegetable)\b/,
  },
  {
    category: "dairy",
    pattern: /\b(milk|cheese|yogurt|egg|butter|cream|dairy)\b/,
  },
  {
    category: "fruit",
    pattern: /\b(apple|banana|orange|berry|fruit|grape|melon|peach|pear)\b/,
  },
  {
    category: "legume_nut",
    pattern: /\b(tofu|bean|nut|almond|walnut|peanut|lentil|soy)\b/,
  },
  {
    category: "beverage",
    pattern: /\b(juice|coffee|tea|water|drink|soda|beverage|milk|smoothie)\b/,
  },
];
...
function guessCategory(foodName: string): string {
  ...
  return (
    CATEGORY_RULES.find(({ pattern }) => pattern.test(lower))?.category ??
    "general"
  );
}
    ...
    try {
      ...
      if (!nutrition) {
        process.stderr.write(
          `[ingest] USDA returned no results for "${trimmed}" — skipping\n`,
        );
        continue;
      }

      const category = categories?.[trimmed] ?? guessCategory(trimmed);
      const entry = mapToCatalogFood(nutrition, results.length, category);
      results.push(entry);
    } catch (err) {
      process.stderr.write(
        `[ingest] Error fetching "${trimmed}": ${errorMessage(err)} — skipping\n`,
      );
    }
  }
...
if (require.main === module) {
  main().catch((err) => {
    process.stderr.write(`[ingest] Fatal: ${errorMessage(err)}\n`);
    process.exit(1);
  });
}
