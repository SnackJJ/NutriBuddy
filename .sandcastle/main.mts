...
// - git identity: the Docker image ships without a default git user, so
//   commits would fail with "Author identity unknown" without this.
// - npm install: ensures platform-specific binaries are present (copyToWorktree
//   seeds node_modules but may miss newly added packages).
const hooks = {
  sandbox: {
    onSandboxReady: [
      {
