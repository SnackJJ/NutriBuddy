...
1. **S1 — 轨迹持久化与 turn 重放**（`docs/rfc/0008`）：路径上的头号阻塞项 —— 审计 / 回放 / 归因 / RL 资产全挂在它上面。**T1–T5 已落地**（迁移 0011、TraceStore 端口、`turn()` 接线与终态语义、`SupabaseTraceStore`、失败语义测试）；剩余 #91（查询面）、#120（迁移 0014）、#122（90 天保留）、#124（D9 越权 smoke）
